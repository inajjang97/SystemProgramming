...
int make_obj(int line){
	...
	//disp계산
	int immediate = asmarr[line].n==0 && asmarr[line].i ==1;
	int indexed = asmarr[line].n==1 && asmarr[line].i ==0;
...
int store_loc(int line, char* m, char* opr, char* opr2){
	...
	Loc[line+1]=Loc[line];
	printf("Loc %d inst %s opr %s opr2 %s adr %04X\n", line,m,opr, opr2, asmarr[line].adr);
	if(strcmp(m,"START")==0){
