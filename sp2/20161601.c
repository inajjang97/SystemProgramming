...
int create_sym( char* name, int adr);
...
int command_asm(char* name){
	...
	//start, end 없는 경우 예외 처리
	if( start_check==0){
		printf("There dose not exist START directive\n");
		return 0;
	}
	if( end_check==0){
	...
	//symbol table 작성
	free_symbol();
	fp_sym=fopen("sym.txt", "r");
	while(fscanf(fp_sym,"%s %d",c, &adr)!=EOF)
		insert_symbol(c,adr);
...
int create_sym(char* name, int adr){
	FILE* fp = fopen("sym.txt", "a+");
	//이미 사용한 symbol 중복으로 한 경우
...
int read_symbol(char* name){
	FILE* fp = fopen("sym.txt", "r");
	char tmp[20];
	int adr;

	while(fscanf(fp,"%s %d", tmp,&adr)!=EOF){
