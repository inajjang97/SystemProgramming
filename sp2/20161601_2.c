...
void copy_symtable(){
	...
	sym* tmp, *node;
	sym* pre_node;
	symtable=(sym**)malloc(sizeof(sym*)*26);
	
	for(i=0; i<26; i++){
		pre_node=NULL;
		for(tmp=symtable_tmp[i]; tmp!=NULL; tmp=tmp->next){
			node= new(tmp->name, tmp->adr);
			if(pre_node==NULL)
				symtable_tmp[i]= node;
...
int insert_symbol(char* name, int adr){
	...
	//처음 symbol 넣는 경우 table size 20 짜리 hash table 할당
	if( symtable_tmp==NULL){
		symtable_tmp=(sym**)malloc(sizeof(sym*)*26);
	}
	//처음 삽입
	if( symtable_tmp[key]==NULL){
		node->next= symtable_tmp[key];
		symtable_tmp[key] = node;
		printf("hihihihihihiih\n");
	}

	//정렬하여 삽입
	else{
		tmp2= NULL;
		for(tmp=symtable_tmp[key]; tmp!=NULL; tmp=tmp->next){
			printf("COMP %s %s\n", name, tmp->name);
...
int create_lst(FILE* fp, char* inst, int line){
	...
	//lc 출력 안해도 되는 경우
	if(inst[0]=='.'|| code_set[line].type==BASE || line == last_line)
		fprintf(fp,"    \t");
	...
	//base 바꿔주는 경우
	if(code_set[line].type==BASE){
		base= read_symbol(operand);
		if(base==-1){
			return 0;
		}
	}
