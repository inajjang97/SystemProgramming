...
int start_check=0, end_check=0, load_check=0;
...
void run_run_run(int end1){
	int rg1, rg2,row, col, op,n,i,x,b,p,e, type, k;
	int operand, ina,song,start,end,end_flag;
	char opr[5];
	char tmp[5];
	...
	while(pc<prog_end){
		...
		end_flag = 0;
		if(!RUN_FLAG){
			for(int j=0;j<bp_index;j++){
				if(start<=bp[j]&&bp[j]<end){
					RUN_FLAG = 1;
					end_flag = 1;
				}
			}
		...
		//STL
			...
			char charr[6];
			sprintf(charr, "%06X", reg[L]);
			for(k=0; k<6; k++){
			
				if( song+k>=32)
					memory[ina+1][(song+k)%32]= charr[k];
				else
					memory[ina][song+k]= charr[k];
		...
		//STCH
			...
			if(song>=32)
				memory[ina+1][song%32] = reg[A]/16;
			else
				memory[ina][song] = reg[A]/16;
			if(song+1>=32)
				memory[ina+1][(song+1)%32] = reg[A]%16;
			else
		...
		//LDCH
			...
			opr[2]='\0';		
			if(song>=32)
				opr[0] = memory[ina+1][song%32];
			else
				opr[0] = memory[ina][song];
			if(song+1>=32)
				opr[1] = memory[ina+1][(song+1)%32];
			else
				opr[1] = memory[ina][song+1];
...
void command_bp(int adr){
	...
	//bp 정렬해서 삽입
	bp[bp_index++]= adr;
	for(i=0; i<bp_index-1; i++){
		for(j=i+1; j<bp_index; j++){
			if( bp[i]>bp[j]){
				int tmp= bp[i];
				bp[i]=bp[j];
				bp[j]=tmp;
