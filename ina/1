...
void add_hash(int key, char *mntemp, char *optemp);
...
void assemble_start(char *filename)
{
	int i, j, s = 0;
	char c;
	char *inp = (char *)malloc(sizeof(char) * 300);
	fp = fopen(filename, "r");
	for(i = 0; i < 100; i++)
		inp[i] = 0;
	for(i = 0; i < 200; i++)
	{
		if(fgets(inp, 300, fp) == NULL)
			break;
		//printf("11111aaaaaaaaaaaaaaaa\n");
		asmarr[i].n = 1;
		asmarr[i].i = 1;
		asmarr[i].x = 0;
		asmarr[i].type = 0;
...
int assemble(char *inp, int line)
{
	char *temp1, *temp2, *temp3, *temp4, *temp5, num = 0;
	char *ptr = (char *)malloc(sizeof(char) * 80);
	char ch;
	int i, j;
	temp1 = (char *)malloc(sizeof(char) * 10);
...
//printf("111111111111bbbbbbbbbbbbbbb\n");
...
//printf("222222222bbbbbbbbbbbbbbb\n");
	while( ptr = strtok(NULL, " ,\n"))
	{
		if(num == 0)
		{
...
//printf("333333333333bbbbbbbbbbbbbbb\n");
	for(i = 0; i < 10; i++)
		title[i] = 0;
	if(line == 0)
	{
...
//printf("44444444444444bbbbbbbbbbbbbbb\n");
	if(line != 0)
	{
...
//printf("555555555555555bbbbbbbbbbbbbbb\n");
